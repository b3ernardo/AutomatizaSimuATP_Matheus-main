...
def conta_tpbig():
    ...
    for proc in psutil.process_iter(attrs=["name"]):
        try:
            if proc.info["name"] and proc.info["name"].lower() == "tpbigm.exe":
                c += 1
        except psutil.Error:
            pass
    return c
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=NUMTHREADS) as executor:
        ...
        for atp_path in atp_files:
            # controla concorrência pelo número de tpbigm.exe
    ...
    # Aguarda encerrar tpbigm.exe remanescentes
    while conta_tpbig() > 0:
        time.sleep(0.5)
